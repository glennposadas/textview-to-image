...
public class JLStickerLabelView: UIView {
    ...
    var labelTextView: JLAttributedTextView?
    ...
    private func setup() {
        ...
        self.rotateGesture = UIRotationGestureRecognizer(target: self, action: #selector(rotatedView(_:)))
        self.addGestureRecognizer(self.rotateGesture)
        
        self.pinchGestureRecognizer = UIPinchGestureRecognizer(target: self, action: #selector(pinchGesture))
...
extension JLStickerLabelView: UITextViewDelegate {
    public func textViewShouldBeginEditing(_ textView: UITextView) -> Bool {
        if textView.text == "Enter text" || textView.attributedText.string == "Enter text" {
            labelTextView!.attributedText = NSAttributedString(string: "", attributes: labelTextView!.textAttributes)
        }
...
extension JLStickerLabelView: UIGestureRecognizerDelegate, adjustFontSizeToFillRectProtocol {
    ...
    @objc func pinchGesture(recognizer: UIPinchGestureRecognizer ) {
        let textView = self.labelTextView!
        ...
        if 1 <= recognizer.scale && recognizer.scale <= 10  {
            self.labelTextView?.fontSize = recognizer.scale * 10
...
extension JLStickerLabelView {
    private func setupLabelTextView() {
        ...
        let attrib = [NSAttributedString.Key.font: UIFont(name: "Verdana-Bold", size: 20.0)!, NSAttributedString.Key.foregroundColor : UIColor.white]
        
        labelTextView?.textAttributes = attrib
        
        labelTextView?.attributedText = NSAttributedString(string: "Enter text", attributes: attrib)
        ...
        labelTextView?.tintColor = UIColor.systemBlue
    ...
    private func setupCloseAndRotateView() {
        closeView = UIImageView(frame: CGRect(x: 0, y: 0, width: 25.0, height: 25.0))
        closeView?.image = UIImage(named: "ic_close")
        ...
        rotateView = UIImageView(frame: CGRect(x: (self.bounds.size.width - globalInset! * 2) + 15.0, y: (self.bounds.size.height - globalInset! * 2) + 10.0, width: 25.0, height: 25.0))
        rotateView?.image = UIImage(named: "ic_rotating")
