...
extension adjustFontSizeToFillRectProtocol {
    func adjustFontSizeToFillRect(_ newBounds: CGRect, view: JLStickerLabelView) {
        ...
        var tempFont: UIFont!
        if labelTextView.textAttributes[NSAttributedString.Key.font] == nil {
            tempFont = UIFont(name: labelTextView.fontName, size: labelTextView.fontSize)
        } else {
            tempFont = labelTextView.textAttributes[NSAttributedString.Key.font] as? UIFont
        }
        ...
        while stickerMinimumFontSize <= stickerMaximumFontSize {
            ...
            tempFont = UIFont(name: tempFont.fontName, size: CGFloat(mid))!
            copyTextAttributes[NSAttributedString.Key.font] = tempFont
